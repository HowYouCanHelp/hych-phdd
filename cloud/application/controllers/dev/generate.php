...
class Generate extends CI_Controller {
	...
	public function jc_generate() {
		$this->from_db();
		$this->create_views_per_folder();
	}
	
	/**
	 * Create a controller and model file for every table in DB with an 'id' column
	 * 
	 * Typical usage:
	 * generate/from_db
	 * 
	 * @return void
	 * @author Jose Carlo Soriano
	 */
	public function from_db() 
	{
		...
		foreach ($tables as $t) {
			/* if ($this->db->field_exists('id', $t)) {
				$this->controller($t, 'MY_Controller', TRUE);
				$this->model($t.'_model');
			} */
			$this->controller($t, 'MY_Controller', TRUE);
			$this->model($t.'_model');
	...
	/**
	 * Creates a folder for every table included in the array with a default set of files
	 * 
	 * Typical usage:
	 * generate/create_views_per_folder
	 * 
	 * @return void
	 * @author Jose Carlo Soriano
	 */
	public function create_views_per_folder() 
	{
		$view_files = array('list', 'indiv', 'add'); //enter the files you want created for each folder
		//$folders = $this->crud_controller; //enter the folders you want created
		$folders = $this->db->list_tables();
	...
	public function controller ($name, $extends = NULL, $crud = FALSE)
	{
		...
		$crud === FALSE || $data .= $this->_crud_methods_contraller($name);
	 ...
	 * @author Joost van Veen
	 */
	private function _crud_methods_contraller ($table = '')
	{
		$string = '';
		$string .= "\n\tpublic function __construct() {\n";
		$string .= "\t\tparent::__construct();\n";
		...
		$string .= "\tpublic function add() {";
		$string .= "\n\t\t\$options = array();";
		$string .= "\n\t\t\$this->crud->add('$table', '$table/add', \$options);";
		$string .= "\n\t}\n\n";
		
		$string .= "\tpublic function edit(\$id) {";
		$string .= "\n\t\t\$options = array();";
		$string .= "\n\t\t\$this->crud->edit(\$id, '$table', '$table/add', \$options);";
		$string .= "\n\t}\n\n";
